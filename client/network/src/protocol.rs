	...
	Client: HeaderBackend<B> + 'static,
{
	...
	pub fn new(
		...
		let protocol = Self {
			...
			need_remote_attestation: network_config.need_remote_attestation.clone(),
		};
	...
	fn on_sync_peer_connected(
		...
		if self.need_remote_attestation {
			...
			let succ = futures::executor::block_on(async {
				...
				(quote_size, quote_buf, sup_size, sup_buf)  = match result {
					Ok(result) => result,
					Err(error) => {
			...
			log::info!("Get Quote from L1 successfully. PeerId:{}", who);
			...
			let mut verify_arg = IoctlVerDCAPQuoteArg {
				...
				supplemental_data_size: sup_size,
				supplemental_data: sup_buf.as_mut_ptr(),
			};
