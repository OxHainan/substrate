...
impl<H, N, S, Id> HistoricalVotes<H, N, S, Id> {
	...
	pub fn new() -> Self {
		HistoricalVotes { seen: Vec::new(), precommit_idx: None }
	}

	/// Create a new HistoricalVotes initialized from the parameters.
	pub fn new_with(seen: Vec<SignedMessage<H, N, S, Id>>, precommit_idx: Option<u64>) -> Self {
		HistoricalVotes { seen, precommit_idx }
	}
