...
use super::{
	BlockNumberOps, Chain, Equivocation, HistoricalVotes, Message, Precommit, SignedMessage,
};
...
pub struct State<H, N> {
	...
	pub precommit_ghost: Option<(H, N)>,
	/// The finalized block.
...
impl<H: Clone, N: Clone> State<H, N> {
	...
	pub fn genesis(genesis: (H, N)) -> Self {
		State {
			precommit_ghost: Some(genesis.clone()),
	...
	Signature: Eq + Clone,
{
	...
	pub fn state(&self) -> State<H, N> {
		State {
			precommit_ghost: self.precommit_ghost.clone(),
	...
	pub fn precommit_ghost(&mut self) -> Option<(H, N)> {
		...
		if self.precommit.current_weight >= threshold {
			self.precommit_ghost = self
				.graph
				.find_ghost(self.precommit_ghost.take(), |v| self.context.weight(v) >= threshold);
		}
	...
	fn update(&mut self) {
		...
		let (g_hash, g_num) = match self.precommit_ghost.clone() {
			None => return,
			Some(x) => x,
		};
		...
		if current_precommits >= self.threshold() {
			self.finalized =
				self.graph.find_ancestor(g_hash.clone(), g_num, |v| ctx.weight(v) >= threshold);
		};
		...
		let possible_to_precommit = {
			...
			move |node: &VoteNode| {
				...
				let precommitted_for = ctx.weight(node);
...
mod tests {
	...
	fn finalization() {
		...
		round
			.import_precommit(&chain, Precommit::new("ED", 10), "Alice", Signature("Alice"))
			.unwrap();
		...
		assert_eq!(round.finalized, Some(("ED", 10)));
	}
	...
	fn historical_votes_works() {
		...
		assert_eq!(
			...
			&HistoricalVotes::new_with(
				vec![SignedMessage {
					message: Message::Precommit(Precommit { target_hash: "EA", target_number: 7 }),
