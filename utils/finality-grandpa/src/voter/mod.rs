...
use crate::{
	...
	HistoricalVotes, Message, Precommit, PrimaryPropose, SignedMessage, LOG_TARGET,
};
	...
	E: Environment<H, N>,
{
	...
	// check threshold support in prevotes and precommits.
	{
		...
		let pc = map.into_iter().fold(VoteWeight(0), |mut pc, (id, precommitted)| {
...
mod tests {
		...
		pool.spawner()
			.spawn(
				round_stream
					.into_future()
					.then(|(value, stream)| {
						assert_eq!(value.unwrap().unwrap().message, precommit);
						let votes = vec![precommit].into_iter().map(Result::Ok);
	...
	fn pick_up_from_prior_with_grandparent_state() {
		...
		for id in 0..67 {
			...
			let msgs = stream::iter(iter::once(Ok(precommit)));
		...
		let last_precommit = Message::Precommit(Precommit { target_hash: "E", target_number: 6 });
		pool.spawner()
			.spawn(
		...
		pool.run_until(
			round_stream
				.skip_while(move |v| {
					...
					if let Message::Precommit(_) = v.message {
