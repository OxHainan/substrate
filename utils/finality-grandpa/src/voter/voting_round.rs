...
use crate::{
	...
	voter_set::VoterSet,
	weights::VoteWeight,
	BlockNumberOps, Commit, HistoricalVotes, ImportResult, Message, Precommit, PrimaryPropose,
	SignedMessage, SignedPrecommit, LOG_TARGET,
};
	...
	N: Copy + BlockNumberOps + ::std::fmt::Debug,
{
	...
	pub(super) fn new(
		...
		VotingRound {
			...
			state: Some(State::Start(round_data.precommit_timer)),
	...
	/// Poll the round. When the round is completable and messages have been flushed, it will return
	/// `Poll::Ready` but can continue to be polled.
	...
	fn primary_propose(&mut self, last_round_state: &RoundState<H, N>) -> Result<(), E::Error> {
		match self.state.take() {
			Some(State::Start(precommit_timer)) => {
				...
				match (maybe_estimate, self.voting.is_primary()) {
					(Some(last_round_estimate), true) => {
						...
						if should_send_primary {
							...
							self.state = Some(State::Proposed(precommit_timer));
				...
				self.state = Some(State::Start(precommit_timer));
			},
			x => {
				self.state = x;
			},
	...
	fn precommit(
		...
		let start_precommit =
			|this: &mut Self, mut precommit_timer: E::Timer, proposed: bool, cx: &mut Context| {
				let should_commit = match precommit_timer.poll_unpin(cx) {
				...
				if should_commit {
					if this.voting.is_active() {
						...
						let (base, mut best_chain) = this.construct_precommit(last_round_state);
						...
						if let Some(target) = best_chain {
							...
							this.env.precommitted(this.round_number(), precommit.clone())?;
							this.votes.set_precommitted_index();
							this.outgoing.push(Message::Precommit(precommit));
						} else {
							warn!(
					target: LOG_TARGET,
					...
					this.state = Some(State::Precommitted);
				} else if proposed {
					this.state = Some(State::Proposed(precommit_timer));
				} else {
					this.state = Some(State::Start(precommit_timer));
				}
				Ok(())
			};
		match state {
			Some(State::Start(timer)) => {
				start_precommit(self, timer, false, cx)?;
			},
			Some(State::Proposed(timer)) => {
				start_precommit(self, timer, true, cx)?;
			},
			x => {
				self.state = x;
			},
	...
	fn construct_precommit(&self, last_round_state: &RoundState<H, N>) -> (H, E::BestChain) {
		...
		let find_descendent_of = match self.primary_block {
			None => last_round_estimate.0,
			Some(ref primary_block) => {
				let last_precommit_g = last_round_state
					.precommit_ghost
					.clone()
					.expect("Rounds only started when prior round completable; qed");

				if primary_block == &last_precommit_g {
					primary_block.0.clone()
				} else if primary_block.1 >= last_precommit_g.1 {
					last_round_estimate.0
				} else {
					let &(ref p_hash, p_num) = primary_block;
					match self
						.env
						.ancestry(last_round_estimate.0.clone(), last_precommit_g.0.clone())
					{
						Ok(ancestry) => {
							let to_sub = p_num + N::one();
							let offset: usize = if last_precommit_g.1 < to_sub {
								0
							} else {
								(last_precommit_g.1 - to_sub).as_()
							};
